...
contract Storage {
    ...
    event StructAdded(uint256 key, address owner, string data, uint256 amount, bool isActive);
    ...
    function addStruct(uint256 key, string memory data, uint256 amount) public {
        MyStruct storage newStruct = myStructs[key];
        newStruct.owner = msg.sender;
        newStruct.data = data;
        newStruct.amount = amount;
        newStruct.isActive = true;

        emit StructAdded(key, msg.sender, data, amount, true);
    }
